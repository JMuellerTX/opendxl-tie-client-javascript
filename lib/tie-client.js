...
const CertProvider = require('./constants/cert-provider')
const CertRepChangeEventProp = require('./constants/cert-rep-change-event-prop')
const CertReputationProp = require('./constants/cert-reputation-prop')
const CertReputationOverriddenProp = require('./constants/cert-reputation-overridden-prop')
const DetectionEventProp = require('./constants/detection-event-prop')
const FileProvider = require('./constants/file-provider')
const FileRepChangeEventProp = require('./constants/file-rep-change-event-prop')
const FirstInstanceEventProp = require('./constants/first-instance-event-prop')
const HashType = require('./constants/hash-type')
const ReputationProp = require('./constants/reputation-prop')
const RepChangeEventProp = require('./constants/rep-change-event-prop')

// Topic used to retrieve the reputation of a file
const TIE_GET_FILE_REPUTATION_TOPIC = '/mcafee/service/tie/file/reputation'
// Topic used to set the reputation of a file
const TIE_SET_FILE_REPUTATION_TOPIC = '/mcafee/service/tie/file/reputation/set'
// Topic used to retrieve systems that have referenced the file
const TIE_GET_FILE_FIRST_REFS_TOPIC = '/mcafee/service/tie/file/agents'
// Topic used to retrieve the reputation of a certificate
const TIE_GET_CERT_REPUTATION_TOPIC = '/mcafee/service/tie/cert/reputation'
// Topic used to set the reputation of a certificate
const TIE_SET_CERT_REPUTATION_TOPIC = '/mcafee/service/tie/cert/reputation/set'
// Topic used to retrieve systems that have referenced the certificate
const TIE_GET_CERT_FIRST_REFS_TOPIC = '/mcafee/service/tie/cert/agents'

// Topic used to notify that a file detection has occurred
const TIE_EVENT_FILE_DETECTION_TOPIC = '/mcafee/event/tie/file/detection'
// Topic used to notify when the first instance of a file has been found
const TIE_EVENT_FILE_FIRST_INSTANCE_TOPIC = '/mcafee/event/tie/file/firstinstance'
// Topic used to notify that a file reputation has changed
const TIE_EVENT_FILE_REPUTATION_CHANGE_TOPIC = '/mcafee/event/tie/file/repchange/broadcast'
// Topic used to notify that a certificate reputation has changed
const TIE_EVENT_CERT_REPUTATION_CHANGE_TOPIC = '/mcafee/event/tie/cert/repchange/broadcast'
...
function transformHashesForServer (obj, hashesKey) {
  ...
  Object.keys(hashesKey).forEach(function (type) {
    obj.hashes.push({ type, value: hexToBase64(hashesKey[type]) })
  })
  return obj
}
...
function transformReputations (reputations) {
  ...
  reputations.forEach(function (reputation) {
    ...
    const overridden = reputation[CertReputationProp.OVERRIDDEN]
    if (overridden && overridden[CertReputationOverriddenProp.FILES]) {
      const overriddenFiles = overridden[CertReputationOverriddenProp.FILES]
...
function transformChangedReputationPayload (repChangePayload) {
  ...
  const newReputations = repChangePayload[RepChangeEventProp.NEW_REPUTATIONS]
  if (newReputations) {
    repChangePayload[RepChangeEventProp.NEW_REPUTATIONS] =
      transformReputations(newReputations.reputations)
  }
  const oldReputations = repChangePayload[RepChangeEventProp.OLD_REPUTATIONS]
  if (oldReputations) {
    repChangePayload[RepChangeEventProp.OLD_REPUTATIONS] =
      transformReputations(oldReputations.reputations)
  }
  const relationships = repChangePayload[FileRepChangeEventProp.RELATIONSHIPS]
  if (relationships) {
    const certificate = relationships.certificate
    if (certificate) {
    ...
    this._dxlClient.asyncRequest(request, function (error, response) {
      ...
      if (response) {
        try {
          ...
          const reputationArr = responseObj.reputations
...
TieClient.prototype.getCertificateReputation =
  function (callback, sha1, publicKeySha1) {
    const payload = publicKeySha1 ? { publicKeySha1: hexToBase64(publicKeySha1) } : {}
    const hashes = {}
